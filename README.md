...
- **Training schedule**: V2 model provides a predefined training schedule. The Trump/Cage results above are generated by model trained for 21k iters using `TOTAL_ITERS = 30000` predefined training schedule.
  - Training trick: Swapping the decoders in late stage of training reduce artifacts caused by the extreme facial expression. e.g., some of the failure cases (of results above) having their mouth open wide are better transformed using this trick.
