...
| Date          | Update        |
| ------------- | ------------- |  
| 2018-06-25      | **Model update**: faceswap-GAN v2.2 has been released. The [v2.2 notebook](https://github.com/shaoanlu/faceswap-GAN/blob/master/FaceSwap_GAN_v2.2_train_test.ipynb) contains code for training and video conversion with face alignment. Additional binary masks are required for training, which can be generated through the [prep_binary_masks](https://github.com/shaoanlu/faceswap-GAN/blob/master/prep_binary_masks.ipynb) notebook. The main improvements of v2.2 model are its capability of generating realistic and consistent eyeballs (results are shown below, or Ctrl+F for eyeballs), as well as its higher quality of video results with face alignment.|
...
- **Consistent eyeballs (v2.2 model):** Results of an updated v2 model which specializes on eyeballs' direcitons are presented below. (Input gifs are created using [DeepWarp](http://163.172.78.19/).) 
...
- **Eyes-aware training:** Introduce reconstruction loss and edge loss around eyes area, which guide the model to generate realistic eyeballs.
