...
class FaceTransformer():
    ...
    def __preprocess_inp_img(self, inp_img, roi_coef, IMAGE_SHAPE):
    ...
    def __ae_forward_pass(self, ae_input):
        ae_out = self.path_func([[ae_input]])
        self.ae_output = np.squeeze(np.array([ae_out]))        
        
    def __postprocess_roi_img(self, ae_output, roi, roi_size, color_correction):
    ...
    def __merge_img_and_mask(self, ae_output_bgr, ae_output_masked, input_size, roi, roi_coef):  
        blend_mask = self.get_feather_edges_mask(roi, roi_coef)      
        ...
        result_alpha[input_size[0]//roi_coef:-input_size[0]//roi_coef, \
                     input_size[1]//roi_coef:-input_size[1]//roi_coef,:] = (blend_mask/255) * self.ae_output_a 
        self.result = result 
    ...
    def transform(self, inp_img, direction, roi_coef, color_correction, IMAGE_SHAPE):
        ...
        self.__preprocess_inp_img(self.inp_img, roi_coef, IMAGE_SHAPE)

        # model inference
        # Set 1 member: self.ae_output
        self.__ae_forward_pass(self.ae_input)
        
        # post-process transformed roi image
        # Set 3 members: self.ae_output_a, self.ae_output_masked, self.ae_output_bgr
        self.__postprocess_roi_img(self.ae_output, self.roi, self.roi_size, color_correction)

        # merge transformed output back to input image
        # Set 3 members: self.result, self.result_rawRGB, self.result_alpha
        self.__merge_img_and_mask(self.ae_output_bgr, self.ae_output_masked, 
                                  self.input_size, self.roi, roi_coef)
